...
GuiWifi::GuiWifi(Window* window) : GuiComponent(window), mMenu(window, "NETWORK SETTINGS"), mVersion(window)
{
	...
	addEntry("CONNECT TO NEW WIFI", 0x777777FF, true, 
		[this, window] { 
			...
			//system("sudo ifconfig2 wlan0 down");
			//system("sudo ifconfig2 wlan0 up");
	...
	addEntry("SHOW CURRENT WIFI INFO", 0x777777FF, true, 
		[this] {
			// dump iwlist and ifconfig2 into a memory file through pipe
			FILE *iwList;
			FILE *wIPP;
			char iw[1035];
			char wip[1035];
			...
			wIPP = popen("ifconfig2 wlan0 | grep 'inet addr'", "r");
